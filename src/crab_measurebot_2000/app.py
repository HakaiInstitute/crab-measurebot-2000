...
def image_to_screen(
    ix: float, iy: float, zoom: float, pan_x: float, pan_y: float
) -> tuple[float, float]:
...
async def export_csv(output_path: Path) -> None:
    measurements = (
        await Measurement.all().select_related("image").order_by("image__path", "id")
    )
...
class ImageCanvas(QWidget):
        ...
        if ev.type() == QEvent.Type.NativeGesture:
            ...
            if isinstance(ev, QNativeGestureEvent):
                if ev.gestureType() == Qt.NativeGestureType.ZoomNativeGesture:
                    self._zoom_at(
                        ev.localPos().x(), ev.localPos().y(), 1.0 + ev.value()
                    )
                    ev.accept()
                    return True
    ...
    def paintEvent(self, event):
        ...
        pw, ph = self._pixmap.width(), self._pixmap.height()
        painter.drawPixmap(
            QRectF(px, py, pw * z, ph * z), self._pixmap, QRectF(self._pixmap.rect())
        )
    ...
    def _draw_overlays(self, painter: QPainter) -> None:
        ...
        for i, m in enumerate(self._state.measurements):
            self._draw_segment(
                ...
                m.y2,
                QColor("#00eeff"),
                f"#{i + 1} · {m.distance_mm:.2f} mm",
            )
        if self._state.pending_point and self._rubber_end:
            p1, p2 = self._state.pending_point, self._rubber_end
            sx1, sy1 = image_to_screen(
                p1[0], p1[1], self._state.zoom, self._state.pan_x, self._state.pan_y
            )
            sx2, sy2 = image_to_screen(
                p2[0], p2[1], self._state.zoom, self._state.pan_x, self._state.pan_y
            )
        ...
        if self._state.mode != "idle":
            mode_text = {
                "placing_iod": "● MEASURE MODE",
                "placing_scale": "● SCALE MODE",
                "panning": "✥ PANNING",
    ...
    def _draw_segment(
        ...
        painter.fillRect(
            QRectF(mx - tw / 2 - 3, my - fm.ascent(), tw + 6, fm.height()),
            QColor(0, 0, 0, 160),
        )
    ...
    def mouseMoveEvent(self, event):
        ...
        ix, iy = screen_to_image(
            sx, sy, self._state.zoom, self._state.pan_x, self._state.pan_y
        )

        if (
            self._state.mode in ("idle", "placing_iod")
            and self._state.drag_start_screen
            ...
            if math.hypot(dx, dy) >= 5:
                self._pre_pan_mode = (
                    "placing_iod" if self._state.mode == "placing_iod" else "idle"
                )
        ...
        if self._state.mode == "panning":
            if (
                self._state.drag_start_pan is None
                or self._state.drag_start_screen is None
            ):
                self._state.mode = self._pre_pan_mode
                self._pre_pan_mode = "idle"
                self.setCursor(Qt.CursorShape.CrossCursor)
            else:
                self._state.pan_x = self._state.drag_start_pan[0] + (
                    sx - self._state.drag_start_screen[0]
                )
                self._state.pan_y = self._state.drag_start_pan[1] + (
                    sy - self._state.drag_start_screen[1]
                )
                self.update()
        ...
        if ir and ir.scale_x1 is not None:
            if (
                distance_point_to_segment(
                    ix, iy, ir.scale_x1, ir.scale_y1, ir.scale_x2, ir.scale_y2
                )
                <= threshold
            ):
...
class RightPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(
            self._section([self._file_label, self._counter_label, self._progress_bar])
        )
        ...
        for key, desc in controls:
            row = QHBoxLayout()
            k = QLabel(key)
            k.setStyleSheet("color:#888;font-size:9px;min-width:40px;")
            d = QLabel(desc)
            d.setStyleSheet("color:#555;font-size:9px;")
            row.addWidget(k)
            row.addWidget(d)
            row.addStretch()
    ...
    def _header(self, text: str) -> QLabel:
        lbl = QLabel(text)
        lbl.setStyleSheet(
            "color:#aaa;font-size:9px;letter-spacing:0.5px;margin-bottom:4px;"
        )
        return lbl
    ...
    def flash_scale_warning(self) -> None:
        ...
        self._scale_status.setStyleSheet(
            "color:#ff6644;font-size:10px;font-weight:bold;"
        )
...
class MainWindow(QMainWindow):
    def __init__(self, image_dir: Path):
        ...
        self._panel.export_requested.connect(
        ...
        QShortcut(QKeySequence(Qt.Key.Key_Left), self, context=ctx).activated.connect(
            lambda: self._navigate(-1)
        )
        QShortcut(QKeySequence("E"), self, context=ctx).activated.connect(
            lambda: asyncio.ensure_future(self._on_export())
        )
        QShortcut(QKeySequence(Qt.Key.Key_Delete), self, context=ctx).activated.connect(
            self._delete_last
        )
        QShortcut(
    ...
    def _delete_last(self) -> None:
        if self._state.measurements:
            asyncio.ensure_future(
                self._do_delete_measurement(self._state.measurements[-1].id)
            )
    ...
    def _navigate(self, delta: int) -> None:
        ...
        self._load_task = asyncio.ensure_future(
            self._load_image(self._state.image_index)
        )
    ...
    async def _on_iod_placed(self, x1: float, y1: float, x2: float, y2: float) -> None:
        ...
        m = await Measurement.create(
            image=ir, x1=x1, y1=y1, x2=x2, y2=y2, distance_mm=dist_mm
        )
    ...
    async def _do_delete_measurement(self, measurement_id: int) -> None:
        ...
        self._state.measurements = [
            x for x in self._state.measurements if x.id != measurement_id
        ]
        self._panel.update_measurements(self._state.measurements)
        self._canvas.update()
    ...
    async def _on_export(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Export CSV",
            str(self._image_dir / "measurements.csv"),
            "CSV Files (*.csv)",
        )
        if path:
